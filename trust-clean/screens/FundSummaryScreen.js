...
export default function FundSummaryScreen() {
  ...
  const handleDownloadPDF = async () => {
    ...
    const token = await AsyncStorage.getItem('token');
    const url = `http://192.168.100.33:8000/api/summary/pdf/?from=${from}&to=${to}`;
    ...
    if (Platform.OS === 'web') {
      try {
        ...
        a.href = downloadUrl;
        a.download = `Summary_${from}_to_${to}.pdf`;
      ...
      try {
        const fileUri = FileSystem.documentDirectory + `Summary_${from}_to_${to}.pdf`;
        ...
        Alert.alert('Download complete', 'Opening summary PDF...');
        ...
        Alert.alert('Download failed', 'Could not download summary PDF.');
      }
    }
  };
